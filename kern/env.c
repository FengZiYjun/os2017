...
env_init(void)
{
	...
	/*
	for(size_t t = 0; t < NENV; t++){

		envs[t].env_status = ENV_FREE;
		envs[t].env_id = 0;

		if(t != NENV - 1){
			envs[t].env_link = &envs[t+1];
		}
	}
	env_free_list = envs;
	*/
	env_free_list = NULL;
	for(size_t i = NENV-1; i+1; --i){
		envs[i].env_id = 0;
...
region_alloc(struct Env *e, void *va, size_t len)
{
	...
	void *begin = ROUNDDOWN(va, PGSIZE), *end = ROUNDUP(va+len, PGSIZE);  // round to align
